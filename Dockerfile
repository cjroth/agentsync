...
#                         override needed.
CMD ["sh", "-c", "VAULT_DIR=\"${AGENTSYNC_CWD:-/data/vault}\" && mkdir -p \"$VAULT_DIR\" && cd \"$VAULT_DIR\" && { [ -f .agentsync/config.toml ] || agentsync init --name \"$AGENTSYNC_VAULT_NAME\" --identity .agentsync/id_ed25519; } && { [ -f .agentsync/id_ed25519 ] || [ -f \"$HOME/.agentsync/id_ed25519\" ] || agentsync key generate --identity .agentsync/id_ed25519; } && exec agentsync watch --listen 0.0.0.0:${PORT:-443}"]
