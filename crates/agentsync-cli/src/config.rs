...
pub fn config_or_global(vault_root: &Path) -> Result<ConfigFile> {
    let local = read_or_default(vault_root)?;
    if has_meaningful(&local) {
        return Ok(local);
    }
    if let Some(home) = dirs_home() {
        let global = home.join(".config/agentsync/config.toml");
        if global.exists() {
            let bytes = std::fs::read(&global)?;
            let s = std::str::from_utf8(&bytes)?;
            let cfg: ConfigFile = toml::from_str(s)?;
            return Ok(merge(local, cfg));
        }
    }
    Ok(local)
}

fn has_meaningful(c: &ConfigFile) -> bool {
    c.vault.id.is_some() || c.vault.rendezvous_url.is_some()
}

fn merge(local: ConfigFile, fallback: ConfigFile) -> ConfigFile {
    ConfigFile {
        vault: VaultSection {
            id: local.vault.id.or(fallback.vault.id),
            rendezvous_url: local.vault.rendezvous_url.or(fallback.vault.rendezvous_url),
        },
        key: KeySection {
            source: local.key.source.or(fallback.key.source),
            keyring_name: local.key.keyring_name.or(fallback.key.keyring_name),
            key_b64: local.key.key_b64.or(fallback.key.key_b64),
            env_var: local.key.env_var.or(fallback.key.env_var),
        },
        sync: local.sync,
    }
}
